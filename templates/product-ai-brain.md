...
**Setup:** Requires one-time Google Calendar OAuth setup. See `prompts/meta/agent-roadmap.md` for detailed instructions.
...
**Setup:** Requires Google OAuth credentials. Run `python scripts/setup_google_oauth.py` to configure, then add the credentials to your environment. See `prompts/meta/agent-roadmap.md` for detailed instructions.
...
- `prompts/pm/ask-dev.md`, `ask-se.md` - Role-play conversations with different functions
...
- **cloudflare-wiki** - Internal wiki for prior decisions and context
- **cloudflare-graphql** - Analytics API schema exploration
